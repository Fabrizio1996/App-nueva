<?php
			...
			if($lang == 'fr') {
				return array(
					 'maintenance'=> array (
					 		...
					 		'content' => "Désolé pour ce désagrément, nous effectuons actuellement des travaux de maintenance. Vous pouvez toutefois <a href=\"mailto:70664972@continental.edu.pe\">nous contacter</a> si besoin, mais nous serons de retour sous peu !",
					 		'sign' => "— L'équipe",
					 ),
					 'order_notify_messages' => array (
							...
							'canceled' => 'Bonjour, votre commande a été annulée.',
						)
				);
			}
			else {
				return array(
					 'maintenance'=> array (
					 		...
					 		'content' => "Sorry for the inconvenience but we&rsquo;re performing some maintenance at the moment. If you need to you can always <a href=\"mailto:70664972@continental.edu.pe\">contact us</a>, otherwise we&rsquo;ll be back online shortly!",
					 		'sign' => "— The Team",
					 ),
					 'order_notify_messages' => array (
							...
							'canceled' => 'Hello, your order has been canceled.',
						)
				);
			}
	  }
	...
	function getOrderStatus( $oid, $default_state = "" ) { 
		$state = $default_state;
		$osfile = ( substr( $oid, -6 ) == ".state" ) ? $oid : "$oid.state";
		if( file_exists( $osfile ) ) {
			$state = file_get_contents( $osfile );
			if( $state == "" ) {
				$state = default_state;
			} else 
				$state = remove_utf8_bom( $state );
		}
		return( $state );
	}	
?>
